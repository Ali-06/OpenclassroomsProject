...
## à configurer voir ##### 7 dans notre exemple : 3 lignes
...
## de la pos 7 : 7+3+1 donc ligne nb=3 ) + \
##### nombre de ports mode trunk si =0 alors pas de création :+ \ 
#####  il y aura autant de lignes qu'il y a de ports trunk à configuer
##### 12 config port mode trunk séparateur "-" si range alors sous la forme + \
##### fa1/1-3-v2(ex: int range fa1/1-3 ...)
...
logging.basicConfig(filename='logcisco.log', level=logging.DEBUG)
   ...
   if int(r1)*int(r2)>0:
    ...
    if int(nbportr)>0:
        ...
        while(k<int(nbportr)):
            ...
            cstr=(asplit[asplit.find("-")+1:len(asplit)])
            cstr=cstr.replace("v", "")
            cstr=cstr.replace("/", "-")]
            confprt=['interface ' + asplit[0:asplit.find("-")], + \ 
            'switchport trunk encapsulation dot1q', + \
            'switchport mode trunk', + \
            'switchport trunk allowed vlan 1-2,'+cstr+',1002-1005']
